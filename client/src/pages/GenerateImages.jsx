...
const GenerateImages = () => {
    ...
    const ImageStyle = [ 'Realistic', 'Ghibli style', 'Realistic style', 'Anime style', 'Cartoon style', 'Fantasy style', 'Portrait style', '3D style']
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
        ...
        <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
            ...
            <textarea onChange={(e)=> setInput(e.target.value)} value={input} type="text" 
            rows={4} placeholder='Describe about the image...'
            ...
            <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
                {ImageStyle.map((item)=> (
                    ...
                    key={item} className={`text-xs px-4 py-1 border rounded-full cursor-pointer
                         ${selectedStyle === item ? 'bg-green-50 text-green-700' 
                    : 'text-gray-500 border-gray-300'}`}>{item}</span>
                ))}
            </div>
